...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [usersRes, cropsRes, analyticsRes, locationRes, typeRes] = await Promise.all([
        ...
        api.get("/api/admin/crops", { headers: { Authorization: `Bearer ${token}` } }),
        api.get("/api/admin/analytics", { headers: { Authorization: `Bearer ${token}` } }),
        api.get("/api/admin/location-analytics", { headers: { Authorization: `Bearer ${token}` } }),
        api.get("/api/admin/type-analytics", { headers: { Authorization: `Bearer ${token}` } }),
      ]);
  ...
  const handleBlockUser = async (id) => {
    await api.patch(`/api/admin/users/${id}/block`, {}, { headers: { Authorization: `Bearer ${token}` } });
    setUsers(users => users.map(u => u._id === id ? { ...u, blocked: true } : u));
  };

  const handleUnblockUser = async (id) => {
    await api.patch(`/api/admin/users/${id}/unblock`, {}, { headers: { Authorization: `Bearer ${token}` } });
    setUsers(users => users.map(u => u._id === id ? { ...u, blocked: false } : u));
  };

  const handleBlockCrop = async (id) => {
    await api.patch(`/api/admin/crops/${id}/block`, {}, { headers: { Authorization: `Bearer ${token}` } });
